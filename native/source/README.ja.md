...
最初のソーススナップショットは、デスクトップアプリの `db/duck.rs` からコピーされました。

ソース SHA-256: `c828ca26090f8699aa2a5e04cca0f328732469aa938b47ecd2bd198877c104c0`。


このディレクトリは、`irodori.duckdb` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。これは、コネクタのランタイム契約がデスクトップアプリに組み込まれるためです。

## マイグレーションスナップショット

| 種類 | ソース | 宛先 | SHA-256 |
|---|---|---|---|
| `desktop-db-adapter` | `apps/desktop/src-tauri/src/db/duck.rs` | `native/source/irodori-table/apps/desktop/src-tauri/src/db/duck.rs` | `c828ca26090f8699aa2a5e04cca0f328732469aa938b47ecd2bd198877c104c0` |
...
エンジンの状態は `knowledge/engines.json` から：`verified`。